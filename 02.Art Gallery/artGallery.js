class ArtGallery {
  ...
  addArticle(articleModel, articleName, quantity) {
    ...
    let existingArticleName = this.listOfArticles.find(
      (x) => x.articleName === articleName
    );

    if (
      existingArticleName &&
      existingArticleName.articleModel === articleModel
    ) {
  ...
  inviteGuest(guestName, personality) {
    let guestPresent = this.guests.find((x) => x.guestName === guestName);
    if (guestPresent) {
  ...
  buyArticle(articleModel, articleName, guestName) {
    ...
    let existingArticleModels = this.listOfArticles.find(
      (x) => x.articleName === articleName
    );

    if (!existingArticleModels) {
      throw new Error("This article is not found.");
    }
    if (existingArticleModels) {
      if (existingArticleModels.articleModel !== articleModel) {
        throw new Error("This article is not found.");
      }
      if (existingArticleModels.quantity === 0) {
    ...
    if (!existingGuest) {
      ...
      if (existingGuest.points >= this.possibleArticles[articleModel]) {
        ...
        existingArticleModels.quantity--;
      } else {
